...
class Lv0:
    ...
    def _is_bz2(self, path: Path) -> bool:
        # True for .../file.fits.bz2 (or any *.bz2)
        return ".bz2" in "".join(path.suffixes)

    def _open_fits_any(self, path: Path):
        """Open .fits or .fits.bz2 and return (hdul, was_bz2_bool)."""
        if self._is_bz2(path):
            with bz2.open(path, "rb") as fobj:
    ...
    def _write_fits_any(self, path: Path, hdu: fits.PrimaryHDU, as_bz2: bool):
        """Write PrimaryHDU to .fits or .fits.bz2."""
        if as_bz2:
            # Write FITS bytes to memory, then bzip2-compress to disk
            buf = io.BytesIO()
            hdu.writeto(buf, overwrite=True)
            data = bz2.compress(buf.getvalue())
    ...
    def update_header(self, fpath_fits):
        ...
        hdul, was_bz2 = self._open_fits_any(fpath)
        hdr = hdul[0].header.copy()
        data = hdul[0].data
