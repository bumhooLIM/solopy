...
class FitsLv1:
    ...
    def correct_bdf(self,
        ...
        try:
            mbias = self._select_master(masterdir, 'BIAS', sci.header.get('JD', 0))
            mdark = self._select_master(masterdir, 'DARK', sci.header.get('JD', 0), sci.header.get('EXPTIME'))
        except Exception as e:
        ...
        try:
            ...
            sci.meta['BIASCORR'] = (True, "Bias corrected?")
            sci.meta['BIASNAME'] = (mbias.meta.get('FILENAME', 'Unknown'), "Master bias frame used")
            
            # Dark
            sci = ccdproc.subtract_dark(sci, mdark, exposure_time="EXPTIME", exposure_unit=u.second)
            sci.meta['DARKCORR'] = (True, "Dark corrected?")
            sci.meta['DARKNAME'] = (mdark.meta.get('FILENAME', 'Unknown'), "Master dark frame used")
            ...
            sci.meta['FLATCORR'] = (True, "Flat corrected?")
            sci.meta['FLATNAME'] = (ccdmflat.meta.get('FILENAME', 'Unknown'), "Master flat frame used")
            
            # mask (bad pixels)
            mask_badpix = (sci.data <= 0) | np.isnan(sci.data) | np.isinf(sci.data) \
                | (ccdmflat.data > 2.0) | (ccdmflat.data <= 0.4) # flat correction can amplify bad pixels, so mask them too
            ...
            sci.meta['HISTORY'] = f"({datetime.now().isoformat()}) BDF corrected and masked. (solopy.FitsLv1)"
            sci.meta['NBADPIX'] = (int(np.sum(combined_mask)), "Number of bad pixels masked")
            sci.meta['MASKNAME'] = (ccdmask.meta.get('FILENAME', 'Unknown') if ccdmask else None, "Master mask frame used")
        ...
        try:
            ra   = int(round(sci.header['RACEN']))
            dec  = int(round(sci.header['DECCEN']))
            pm   = 'p' if dec >= 0 else 'n'
            exp  = int(round(sci.header['EXPTIME']))
            obst = Time(sci.header['DATE-OBS']).strftime("%Y%m%d%H%M%S")
            fname_out = f"kl4040.sci.lv1.{ra:03d}.{pm}{abs(dec):02d}.{exp:03d}.{obst}.fits"
            sci.meta['FILENAME'] = fname_out
        except KeyError:
        ...
        primary_hdu = fits.PrimaryHDU(data=sci.data, header=sci.header)
